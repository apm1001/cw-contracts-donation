use cosmwasm_std::{ensure, DepsMut, Env, MessageInfo, Response, StdResult, Uint128, WasmMsg, coins, to_json_binary, SubMsg};
use cw_utils::must_pay;

use crate::contract::WITHDRAW_REPLY_ID;
use crate::error::ContractError;
use crate::msg::{DistribtionExecMsg, MembershipExecMsg};
use crate::state::{CONFIG, OWNER, DONATIONS, HALFTIME, LAST_UPDATED, WEIGHT, PENDING_WITHDRAWAL, WithdrawalData};
...
pub fn donate(deps: DepsMut, info: MessageInfo) -> Result<Response, ContractError> {
    ...
    DONATIONS.update(deps.storage,
      |donations| -> StdResult<_> {
        Ok(donations + 1)
      }
    )?;
...
pub fn withdraw(
    ...
    PENDING_WITHDRAWAL.save(
      deps.storage,
      &WithdrawalData { receiver, amount }
    )?;
    ...
    let withdraw_msg = WasmMsg::Execute {
        contract_addr: config.distribution_contract
          .into_string(),
...
pub fn propose_member(deps: DepsMut, info: MessageInfo, addr: String) -> Result<Response, ContractError> {
    let owner = OWNER.load(deps.storage)?;
    ensure!(owner == info.sender, ContractError::Unauthorized);
    ...
    let resp = Response::new()
        ...
        .add_attribute("sender", info.sender.as_str())
        .add_attribute("proposed_member", addr);
