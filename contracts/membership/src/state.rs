...
pub const CONFIG: Item<Config> =
  Item::new("config");
pub const MEMBERS: Map<&Addr, Empty> =
  Map::new("members");
// (member_voter, candidate)
pub const VOTES: Map<(&Addr, &Addr), Empty> =
  Map::new("votes");
pub const PROPOSALS: Map<&Addr, u64> =
  Map::new("proposals");
 
pub const AWAITING_INITIAL_RESPS: Item<u64> =
  Item::new("awaiting_initial_resps");